...
// WithProduct stores project in context.
func WithProduct(ctx context.Context, p *Product) context.Context {
	key := projectKey
	return context.WithValue(ctx, key, p)
}

// ProductFromContext retrieves project from context.
func ProductFromContext(ctx context.Context) (*Product, bool) {
	key := projectKey
	v, ok := ctx.Value(key).(*Product)
	return v, ok
}
