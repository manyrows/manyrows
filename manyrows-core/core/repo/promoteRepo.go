...
func (r *Repo) PromoteApp(
	...
	const configQ = `
		INSERT INTO config_values (
			id, product_id, app_id, config_key_id,
			value_json, value_encrypted,
			updated_at, updated_by_account_id
		)
		SELECT
			gen_random_uuid(), cv.product_id, $3, cv.config_key_id,
			cv.value_json, cv.value_encrypted,
			$5, $4
		FROM config_values cv
		JOIN apps src ON src.id = $2 AND src.product_id = $1
		JOIN apps tgt ON tgt.id = $3 AND tgt.product_id = $1
		WHERE cv.app_id = $2
		  AND cv.product_id = $1
	...
	tag, err := tx.Exec(ctx, configQ, productID, sourceAppID, targetAppID, actorAccountID, now)
	if err != nil {
		return result, err
	}
	...
	const flagQ = `
		INSERT INTO feature_flag_overrides (
			id, product_id, app_id, feature_flag_id,
			enabled, status,
			updated_at, updated_by_account_id
		)
		SELECT
			gen_random_uuid(), ffe.product_id, $3, ffe.feature_flag_id,
	...
	tag, err = tx.Exec(ctx, flagQ, productID, sourceAppID, targetAppID, actorAccountID, now)
	if err != nil {
		return result, err
	}
