...
// HandleUpdateAppOIDCConfig is PUT /admin/.../products/{pid}/apps/{appId}/oidc-config.
...
func (handler *RequestHandler) HandleUpdateAppOIDCConfig(w http.ResponseWriter, r *http.Request) {
	...
	curApp, err := handler.repo.GetAppByIDForProduct(r.Context(), ws.ID, productID, appID)
	if err != nil {
...
// HandleGetAppOIDCConfig is GET /admin/.../products/{pid}/apps/{appId}/oidc-config.
...
func (handler *RequestHandler) HandleGetAppOIDCConfig(w http.ResponseWriter, r *http.Request) {
	...
	app, err := handler.repo.GetAppByIDForProduct(r.Context(), ws.ID, productID, appID)
	if err != nil {
		if errors.Is(err, repo.ErrNotFound) {
			WriteError(w, r, "error.appNotFound", http.StatusNotFound)
			return
