...
// the operator hunting through every product. Each row carries the
...
// POST /admin/workspace/{workspaceId}/products/{productId}/apps/{appId}/userPool
func (handler *RequestHandler) HandleRepointAppUserPool(w http.ResponseWriter, r *http.Request) {
	// adminAndProduct validates the (workspace, project) pair belongs
	...
	_, ws, project, ok := handler.adminAndProduct(w, r)
	if !ok {
		return
	}
	...
	app, err := handler.repo.GetAppByIDForProduct(r.Context(), ws.ID, project.ID, appID)
	if err != nil {
		if errors.Is(err, repo.ErrNotFound) {
			WriteError(w, r, "error.notFound", http.StatusNotFound)
			return
