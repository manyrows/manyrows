...
// PUT /admin/workspace/{ws}/products/{p}/apps/{a}/cookie-domain.
//
...
func appCookieDomainURL(wsID, projID, appID string) string {
	return "/admin/workspace/" + wsID + "/products/" + projID + "/apps/" + appID + "/cookie-domain"
}
...
func TestAppCookieDomain_OverridesWorkspace(t *testing.T) {
	...
	router, wsRouter := NewAdminWorkspaceRouter(t, svc)
	wsRouter.Put("/cookie-domain", svc.Handler.HandleUpdateWorkspaceCookieDomain)
	wsRouter.Put("/products/{productId}/apps/{appId}/cookie-domain", svc.Handler.HandleUpdateAppCookieDomain)
	...
	// Set app override.
	rr := putCookieDomain(t, router, appCookieDomainURL(ws.ID.String(), app.ProductID.String(), app.ID.String()), claims,
