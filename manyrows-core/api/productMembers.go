...
type ProductMembersResponse struct {
...
// HandleGetProductMembers returns ONLY members that have at least one role
...
func (handler *RequestHandler) HandleGetProductMembers(w http.ResponseWriter, r *http.Request) {
	...
	if appID != uuid.Nil && len(members) > 0 {
		...
		if counts, err := handler.repo.GetProductMembersActivityCounts(r.Context(), appID, userIDs); err == nil {
			for i := range members {
