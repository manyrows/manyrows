...
// GetProductAsAdmin returns a project if the admin has access to the workspace.
// Relies on middleware having already validated workspace ownership.
func (handler *RequestHandler) GetProductAsAdmin(
	ctx context.Context,
	id uuid.UUID,
	workspaceID uuid.UUID,
	acc *core.Account,
) (*core.Product, bool, error) {
	...
	project, err := handler.repo.GetProduct(ctx, id, ws.ID)
	if err != nil {
		return nil, false, err
	}
	if project == nil {
