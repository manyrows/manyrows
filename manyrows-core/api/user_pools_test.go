...
func TestSharedUserPool_OneIdentityAcrossTwoApps(t *testing.T) {
	...
	// sibling product (different product_id) so the (product_id, type)
	// unique constraint doesn't fire when both are type=dev - and that
	// matches the canonical "Acme dashboard + Acme marketing share
	// users across two products" SSO case.
	appBProductID := utils.NewUUID()
	if _, err := testEnv.DB.Pool().Exec(ctx, `
		INSERT INTO products (id, workspace_id, name, created_at, updated_at, created_by_account_id)
		VALUES ($1, $2, $3, NOW(), NOW(), $4)
	`, appBProductID, ws.ID, "Shared Sibling "+GenerateUniqueSlug("p"), acc.ID); err != nil {
		t.Fatalf("insert sibling product: %v", err)
	}
	appBID := utils.NewUUID()
	if _, err := testEnv.DB.Pool().Exec(ctx, `
		INSERT INTO apps (id, workspace_id, product_id, user_pool_id, type, enabled, primary_auth_method, created_at, updated_at)
		VALUES ($1, $2, $3, $4, 'dev', true, 'password', NOW(), NOW())
	`, appBID, ws.ID, appBProductID, appA.UserPoolID); err != nil {
...
func TestGetProductMembersByApp_IncludesRolelessMember(t *testing.T) {
	ctx := context.Background()
	...
	members, total, err := testEnv.Repo.GetProductMembersByApp(ctx, app.ProductID, app.ID, 0, 100, "", 0, repo.MemberEnabledFilterAny, repo.MemberRoleFilter{})
	if err != nil {
		t.Fatalf("GetProductMembersByApp: %v", err)
	}
	if total < 1 {
		t.Errorf("expected total >= 1, got %d", total)
	}
	if !containsMember(members, email) {
		t.Errorf("roleless member %q missing from GetProductMembersByApp", email)
	}
}
...
func TestUpdateAppRegistration_NoDefaultRoleAllowed(t *testing.T) {
	...
	updated, err := testEnv.Repo.UpdateAppRegistration(ctx, ws.ID, app.ProductID, app.ID, repo.AppRegistrationUpdate{
...
func allowReg(t *testing.T, app *core.App, on bool) *core.App {
	...
	updated, err := testEnv.Repo.UpdateAppRegistration(context.Background(),
		app.WorkspaceID, app.ProductID, app.ID, repo.AppRegistrationUpdate{
