...
// run. The product is the user-named entity; each app under it is an
...
export default function QuickStartWizard({ open, onClose, workspace, onComplete }: Props) {
  ...
  const handleCreate = async () => {
    ...
    setError(null);
    try {
      // 1. Product carries the user-chosen name. The app under it
      //    inherits identity from this label (display name is
      //    composed from product + env type at read time).
      const productRes = await axios.post<{ id: string }>(`${basePath}/products`, {
        name: trimmedName,
      });
      ...
      await axios.post(`${basePath}/products/${productId}/apps/`, {
        type: appEnv,
        enabled: true,
        appUrl: trimmedUrl,
    ...
    <Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm">
      ...
      <DialogContent>
        <Stack spacing={2.25} sx={{ mt: 1 }}>
          ...
          <TextField
            label={t("wizard.productName.label", { defaultValue: "Product name" })}
            ...
            size="small"
            disabled={loading}
            helperText={t("wizard.productName.help", {
              defaultValue: "The product this belongs to. Each environment - prod, staging, dev - is a separate app under it.",
            })}
          />
          ...
          <FormControl size="small" fullWidth disabled={loading}>
            ...
            <FormHelperText>
              {t("wizard.appEnv.help", {
                defaultValue:
                  "Add the other environments later - prod / staging / dev share roles, permissions, config keys, and feature flags under one product.",
              })}
            </FormHelperText>
          </FormControl>
