...
const SCORE_LABELS = [
  { value: 0, label: "0 - Anything goes" },
  { value: 1, label: "1 - Reject obvious passwords" },
  { value: 2, label: "2 - Safe-ish (recommended)" },
  ...
  return (
    <Stack spacing={3} sx={{ maxWidth: 680 }}>
      <Typography variant="body2" color="text.secondary" sx={{ maxWidth: 620 }}>
        Applied when users set or reset a password. Existing passwords
        aren't re-checked - the policy only gates new ones.
      </Typography>

        <Alert severity="info" sx={{ fontSize: 13 }}>
          We don't expose the deprecated "must contain uppercase / digit /
          symbol" knobs. Strength is scored with zxcvbn, which catches
          dictionary words, keyboard patterns, and personal-info reuse -
          the categories that actually fall to attacks.
        </Alert>

        <Box>
          <Stack direction="row" alignItems="baseline" justifyContent="space-between" sx={{ mb: 1 }}>
            <Eyebrow>Minimum length</Eyebrow>
          ...
          <Typography variant="caption" color="text.secondary" sx={{ display: "block" }}>
            NIST recommends a floor of 8. 12+ is stronger and still
            survivable for memorisation; 16+ assumes a password manager.
          </Typography>
        </Box>

        <Box>
          <Eyebrow sx={{ mb: 1 }}>Minimum strength score (zxcvbn)</Eyebrow>
          <TextField
            select
            value={minScore}
          ...
          <Typography variant="caption" color="text.secondary" sx={{ display: "block", mt: 1 }}>
            Score 2 rejects "password123", "qwerty", and trivial substitutions.
            Score 3+ starts blocking many human-memorable passwords - pair
            with a password-manager-only audience.
          </Typography>
        </Box>
