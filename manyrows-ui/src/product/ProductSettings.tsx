...
interface Props {
  project: Product;
  onUpdated?: (project: Product) => void;
}
...
async function updateProduct(
  ...
  return axios
    .put(`/admin/workspace/${workspaceId}/products/${productId}`, body)
    .then((r) => r.data)
...
export default function ProductSettings(props: Props) {
  ...
  const confirmDelete = async () => {
    setDeleting(true);
    try {
      await axios.delete(`/admin/workspace/${project.workspaceId}/products/${project.id}`);
      enqueueSnackbar(
        t("projectSettings.deleted", { defaultValue: "Product deleted" }),
        { variant: "success" },
      );
  ...
  const onSave = async () => {
    ...
    setSaving(true);
    try {
      const updated = await updateProduct(project.workspaceId, project.id, { name: nameTrim });
  ...
  return (
    <Box>
              ...
              <Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems={{ sm: "center" }} justifyContent="space-between">
                <Box>
                  <Typography variant="body2" sx={{ fontWeight: 500 }}>
                    {t("projectSettings.deleteTitle", { defaultValue: "Delete this product" })}
                  </Typography>
                  <Typography variant="caption" color="text.secondary">
                    {t("projectSettings.deleteHint", { defaultValue: "Removes the product, its apps, and all associated data. This cannot be undone." })}
                  </Typography>
                </Box>
                <Button
                  color="error"
                  ...
                  sx={{ flexShrink: 0 }}
                >
                  {t("projectSettings.deleteButton", { defaultValue: "Delete product…" })}
                </Button>
              </Stack>
            </Stack>
          </CardContent>
      ...
      <Dialog open={deleteOpen} onClose={() => !deleting && setDeleteOpen(false)} fullWidth maxWidth="xs">
        <DialogTitle>
          {t("projectSettings.deleteDialogTitle", { defaultValue: "Delete product" })}
        </DialogTitle>
        <Box
          component="form"
          onSubmit={(e) => {
          ...
          <DialogContent sx={{ pt: 1 }}>
            <Stack spacing={1.5}>
              <Alert severity="warning">
                {t("projectSettings.deleteWarning", { defaultValue: "This permanently deletes the product, its apps, users, and roles. This action cannot be undone." })}
              </Alert>
